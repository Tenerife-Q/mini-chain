...
### 餐厅点菜比喻
- **`unwrap()`（原方案）**：就餐时发现菜里有虫子，顾客直接掀翻桌子（`Panic` 闪退），整个餐厅强制停业。这在任何在线服务中都是极其恶劣的。
- **`?` 问号语法糖（现方案）**：服务员发现菜有问题，自己决定不了，他直接在点菜单上画个 `?`，把问题递飞快地递交给大堂经理（把异常抛给上层调用者）。
- **`match`（兜底方案）**：大堂经理（主函数 `main`），拿到抛上来的问题，冷静决定是给顾客退款、换菜，还是仅仅记个账（降级容错），无论如何，餐厅继续正常营业。

### 万能复制模板：日常业务逻辑层
在 PingCAP 等企业级开发中，当我们处理一段涉及“查库、网络请求、写文件”的组合拳时，直接套用这个模板，可以应对 99% 的场景：

```rust
// 【万能返回签名】：Result<你本来想要的最终类型, 动态错误盒子>
pub fn process_business_logic(user_id: u64) -> Result<String, Box<dyn std::error::Error>> {
    // 步骤 1: 读取本地缓存，可能文件不存在，用 ? 甩锅
    let raw_data = std::fs::read_to_string("user_db.json")?; 
    
    // 步骤 2: 解析 JSON，可能格式损坏，用 ? 甩锅
    // 注意：serde_json 报错类型和 fs 报错类型完全不同，但 Box<dyn Error> 能把它们统统包容装下！
    let parsed_user: User = serde_json::from_str(&raw_data)?;
    
    // 步骤 3: 提取用户需要的数据，若没有就抛出自定义文字错误
    let name = parsed_user.name.ok_or("用户未设置姓名规则")?;
    
    // 全程没触发 ?（即全都是 Ok 顺利过关），最后将成功结果包在 Ok 中返回
    Ok(name)
}
```
